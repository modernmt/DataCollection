...
my $HELP = 0;
my $NUM_THREADS = 1;
my $SPLIT_ON_LINEBREAK = 0;

while (@ARGV) {
        $_ = shift;
        /^-l$/ && ($language = shift, next);
        /^-threads$/ && ($NUM_THREADS = int(shift), next);
        /^-splitlinebreak$/ && ($SPLIT_ON_LINEBREAK = 1, next);
        /^-q$/ && ($QUIET = 1, next);
        /^-h$/ && ($HELP = 1, next);
}

if ($HELP) {
    print "Usage ./split-sentences.perl [-l [en|de|...]] [-threads N ] [-splitlinebreak] [-q] [-h] < textfile > splitfile\n";
        exit;
}
...
if (-e "$prefixfile") {
        print STDERR "Loading prefixes from: $prefixfile\n";
